...
const UPCOMING_HOLIDAYS: Holiday[] = [
  ...
  { id: "h_civic_on", holidayDate: "2026-08-03", description: "Civic Holiday (Ontario)", scope: "all" },
];
...
const PAST_HOLIDAYS: Holiday[] = [
  ...
  { id: "h_family", holidayDate: "2026-02-16", description: "Family Day (Ontario)", scope: "all" },
  { id: "h_ny", holidayDate: "2026-01-01", description: "New Year's Day", scope: "all" },
];

const SCOPE_OPTIONS = [
  { value: "all", label: "All Locations" },
  { value: "loc_mla_toronto", label: "Toronto Downtown only" },
  { value: "loc_mla_mississauga", label: "Mississauga only" },
  { value: "loc_mla_brampton", label: "Brampton only" },
];

type AddForm = { holidayDate: string; description: string; scope: string };
const BLANK_FORM: AddForm = { holidayDate: "", description: "", scope: "all" };
...
export default function FranchiseeHolidaysPage() {
  ...
  return (
    <div style={{ padding: 24 }}>
      <PageHeader
        ...
        subtitle="Maple Leaf Academy — manage location-specific holidays"
        actions={
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {upcoming.map((h) => {
            ...
            return (
              <Card
                key={h.id}
                style={{ padding: "16px 20px", border: isImminent ? `1.5px solid ${TLP.amber}` : undefined }}
              >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
                  <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 12, color: TLP.gray500, marginTop: 2 }}>{fmtDate(h.holidayDate)}</div>
        ...
        {showPast && (
          <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 8 }}>
            {PAST_HOLIDAYS.map((h) => (
              ...
              >
                <div>
                  <span style={{ fontWeight: 600, fontSize: 13, color: TLP.gray700 }}>{h.description}</span>
                  <span style={{ color: TLP.gray400, fontSize: 12, marginLeft: 10 }}>{fmtShortDate(h.holidayDate)}</span>
                </div>
                <Badge label={scopeLabel(h.scope)} color={TLP.gray500} bg={TLP.gray100} />
              </div>
            ))}
          </div>
        )}
      ...
      >
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <Input label="Date" type="date" value={form.holidayDate} onChange={set("holidayDate")} required />
          <Input
            label="Description"
