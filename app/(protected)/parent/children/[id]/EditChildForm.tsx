...
export default function EditChildForm({ student }: { student: Student }) {
  ...
  async function handleDelete() {
    if (!confirm(`Remove ${student.full_name} from your account? This cannot be undone.`)) return;
    ...
    <form onSubmit={handleSubmit} className="bg-card-dark border border-border-dark rounded-xl shadow-sm p-6 space-y-5">
      ...
      <div className="border-t border-border-dark pt-5">
        <button
          ...
          className="flex items-center gap-2 text-error hover:text-red-400 text-sm font-medium transition-colors disabled:opacity-50"
        >
          <span className="material-icons-round text-[18px]">delete</span>
          {deleteLoading ? "Removing…" : "Remove child from account"}
        </button>
      </div>
    </form>
  );
