...
export default async function CoursesPage({
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-white">Course Browser</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {courses.map((course: any) => (
            <Link
              ...
              className="bg-card-dark border border-border-dark rounded-xl p-5 hover:border-primary/40 hover:bg-card-hover transition-all duration-200 group flex flex-col"
            >
              {/* Header */}
              <div className="flex items-start justify-between gap-2 mb-3">
                <span
                  className={`text-xs font-semibold px-2 py-0.5 rounded-full capitalize ${TYPE_COLORS[course.type] ?? "bg-slate-700 text-slate-300"}`}
                >
                  {TYPE_LABELS[course.type] ?? course.type}
                </span>
                {course.skill_level && (
                  <span
                    className={`text-xs font-semibold px-2 py-0.5 rounded-full capitalize ${SKILL_COLORS[course.skill_level] ?? "bg-slate-700 text-slate-300"}`}
                  >
                    {course.skill_level}
                  </span>
                )}
              ...
              {/* Meta */}
              <div className="space-y-1.5 text-sm text-slate-400 border-t border-border-dark pt-4">
              ...
              <div className="mt-4 flex items-center justify-between">
                <div>
                  {Number(course.price_trial) === 0 ? (
                    ...
                    <span className="text-slate-300 text-sm font-semibold">${course.price_trial} trial</span>
                  )}
