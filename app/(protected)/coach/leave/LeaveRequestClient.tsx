    ...
    <div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-card-dark border border-primary/30 rounded-xl p-5 mb-6">
          <h3 className="text-white font-bold mb-4">Request Leave</h3>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="text-slate-400 hover:text-white px-4 py-2 text-sm transition-colors border border-border-dark rounded-lg"
            >
              Cancel
            </button>
          </div>
      ...
      {leaves.length === 0 ? (
        <div className="bg-card-dark border border-border-dark rounded-xl p-8 text-center">
          ...
          <p className="text-slate-400 text-sm mt-1">You haven't requested any time off yet.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {leaves.map((l) => (
            <div key={l.id} className="bg-card-dark border border-border-dark rounded-xl p-5 flex flex-col">
              ...
              <p className="text-slate-400 text-sm flex-1">{l.reason || "No reason provided"}</p>
              
              {l.admin_note && (
                <div className="mt-4 pt-4 border-t border-border-dark">
                  <p className="text-[11px] text-slate-500 uppercase font-bold mb-0.5">Admin Note</p>
                  <p className="text-xs text-slate-300">{l.admin_note}</p>
                </div>
              )}
            </div>
          ))}
